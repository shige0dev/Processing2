...
void setupGUI() {
  ...
  for (int j = 0; j < 6; j++) {
    cp5.addBang("bang"+j)
      .setPosition(width / (22 + 1) * 5 - toggleSize.x / 2, height / 12 * (j+5) - toggleSize.y / 2)
    ...
    for (int i = 0; i < 16; i++) {
      toggles[j][i] = cp5.addToggle("sound"+ j +""+ (i + 1))
        .setCaptionLabel("")
        .setValue(false)
        .setPosition(width / (22 + 1) * (i + 6) - toggleSize.x / 2, height / 12 * ( j + 5) - toggleSize.y / 2)
        .setLabel("")
