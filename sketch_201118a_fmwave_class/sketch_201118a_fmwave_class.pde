...
void draw(){
  //mouse value
  voice1.LEVEL_B = float(mouseY)/height; 
  
  //sound refresh
  sound_val  = voice1.update();
  //sound_val += voice2.update();
  //sound_val += voice3.update();
  //sound_val /=3;
  ...
  stroke(255,0,0);
  point(width-10, voice1.adsr0.val * (-50) +   height/4 );
  stroke(0,255,0);
  point(width-10, voice1.adsr1.val * (-50) + 2*height/4 );
  stroke(0,0,255);
  point(width-10, voice1.adsr2.val * (-50) + 3*height/4 );
...
void keyPressed(){
   ...
   if(voice1.gate_on(int(key)))println("gateOn");
...
void keyReleased(){
  if(voice1.gate_off(int(key)))println("gateOff");
