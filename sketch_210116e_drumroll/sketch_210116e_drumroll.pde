...
Interval =
  cp5.addKnob("Interval")
     ...
     .setRange(1, 120)
     .setValue(30)
     .setPosition(width * 3.0 / 5.0 - radius, height * 2.8 / 3.0 - radius)
     .setRadius(radius)
     .setDragDirection(Knob.VERTICAL);     
gateTime =
  cp5.addKnob("GateTime")
     ...
     .setRange(0.01,1.0)
     .setValue(0.1)
     .setPosition(width * 2.0 / 5.0 - radius, height * 2.8 / 3.0 - radius)
     .setRadius(radius)
     .setDragDirection(Knob.VERTICAL);   
}
...
void draw(){
 ...
 if( count > Interval.getValue()){
   count = 0;
   out.playNote(0.0, gateTime.getValue() , new DrumInstrument());
 }
}
...
class DrumInstrument implements Instrument {
  ...
  DrumInstrument(){
    sine = new Oscil(sinPitch, 1.0, Waves.SQUARE);
    noise = new Noise(1.0);
