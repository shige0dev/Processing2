...
void setup(){
  ...
  delay = new Delay(1.0, 0.9, true, true);
...
// functions for VCO
void setVcoWave(int _value){

   switch(_value){
     case 0:
     oscil1.setWaveform( Waves.SINE);    println("SINE");
     break;
     
     case 1:
     oscil1.setWaveform( Waves.TRIANGLE); println("Triangle");
     break;
     
     case 2:
     oscil1.setWaveform( Waves.SQUARE); println("SQUARE");
     break;
     
     case 3:
     oscil1.setWaveform( Waves.SAW);   println("SAW");
     break;
     
     case 4:
     oscil1.setWaveform(Waves.QUARTERPULSE);   println("SAW");
     break;
   } 
}
...
void setCutoffFrequency(float _value){
  vcfLfo.amplitude.setLastValue( _value / 2.0 * vcfLfoIntensityKnob.getValue( ) );
//  println(vcfLfoIntensityKnob.getValue());
//  vcfLfo.amplitude.setLastValue(_value / 2.0);
  vcfLfo.offset.setLastValue( _value);
}

void setResonance(float _value){
  filter.resonance.setLastValue( _value);
}

// function for VCA + EG

void setAmplitude(float _value){
  vcaLfo.amplitude.setLastValue( _value / 2.0 * vcaLfoIntensityKnob.getValue() );
  vcaLfo.offset.setLastValue( _value );
}
...
// functions for LFO
void setLfoWave(boolean _value){
   if(_value){
...
void setLfoRate(float _value){
  vcoLfo.setFrequency(_value);
  vcfLfo.setFrequency(_value);
  vcaLfo.setFrequency(_value);
}

void setVcfIntensity(float _value){
  vcfLfo.amplitude.setLastValue(cutoffFrequencyKnob.getValue() / 2.0 * _value);
}

void setVcaIntensity(float _value){
  vcaLfo.amplitude.setLastValue(amplitudeKnob.getValue() / 2.0 * _value);
}
